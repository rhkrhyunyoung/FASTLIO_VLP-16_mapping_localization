...
This repository is a modified version of hku-mars/FAST_LIO (ROS2), optimized for broader LiDAR compatibility and localization capabilities.

    Removed Livox SDK Dependency: Original FAST_LIO requires Livox drivers even when using Velodyne LiDARs. This version has been modified to build and run seamlessly using Velodyne drivers only, without needing the Livox-related environment.
...
3. Save Map(map_file_path is in config/velodyne.yaml)
...
For the odom → base_link → sensor_frame structure required by Nav2, the tf structure is organized through localization.yaml modification.
